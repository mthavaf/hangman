...
def main():
        ...
        conn = pymysql.connect(host="localhost",user="root",passwd="admin123",db="hangman")
        cur = conn.cursor()
        print ('you have 1:simple,2:medium and 3:complex\n')
        level = int(input('select the level you want to play\n'))
        if level == 1:
                cur.execute("SELECT words FROM easy ORDER BY RAND() LIMIT 1")
                data = cur.fetchone()
                data = data[0]       
        elif level == 2:
                cur.execute("SELECT words FROM moderate ORDER BY RAND() LIMIT 1")
                data = cur.fetchone()
                data = data[0]
        elif level == 3:
                cur.execute("SELECT words FROM difficult ORDER BY RAND() LIMIT 1")
                data = cur.fetchone()
                data = data[0]
        else:
