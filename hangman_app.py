...
def main(char, wrongAttempts, array, data, guessedWord, level):
    ...
    while (wrongAttempts != 0 and "-" in guessedWord):
        ...
        w.create_text(250, 50, font=("times roman", 30),
                      text="HANGMAN", fill="#B0171F")
        ...
        Button(master, text='enter', command=lambda: main(textBox.get(),
               wrongAttempts, array, data, guessedWord, level)).place(x=750, y=300)
        if(wrongAttempts == 0):
        ...
        Button(master, text='newgame', command=lambda: init(
            master, w, level)).place(x=830, y=300)
        mainloop()
...
if __name__ == "__main__":
    ...
    w.create_text(250, 50, font=("times roman", 30),
                  text="HANGMAN", fill="#B0171F")
    Button(master, text='simple', command=lambda: init(
        master, w, 1)).place(x=700, y=50)
    Button(master, text='moderate', command=lambda: init(
        master, w, 2)).place(x=800, y=50)
    Button(master, text='difficult', command=lambda: init(
        master, w, 3)).place(x=920, y=50)
    mainloop()
